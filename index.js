...
//Black Beys
const blackbeyFiles = fs.readdirSync('/home/container/blackbeys').filter(file => file.endsWith(".js") && file !== "Beyblade.js");
for (const file of blackbeyFiles) {
    const blackbey = require(`/home/container/blackbeys/${file}`);
...
client.on('messageCreate', async (message) => {
    ...
    const testForNumber2 = Math.floor(Math.random() * 1);
    ...
    if (testForNumber === 0) {
        ...
        if (raritytest >= 20) {
            ...
            available = ["Judgement Joker", "Screw Trident", "Wyvern", "Buster Xcalibur", "Hyrus H2", "Kerbeus", "Killer Deathscyther", "Evileye", "ChoZ Valkyrie", "Nightmare Longins", "Spriggan", "Revive Phoenix", "Sieg Xcalibur", "Rock Leone", "Draciel Shield", "Xcalibur", "Dead Phoenix", "Lost Longinus"];
            ...
            available = ["Victory Valkyrie", "Dragoon Storm", "Nova Neptune", "Shelter Regulus", "Lightning L Drago", "Guardian Kerbeus", "Bloody Longinus", "Earth Aquila", "Yaeger Yggdrasil", "Trident", "Tyros T2", "Mad Minoboros", "Unicorn", "Dranzer Spiral", "Obelisk Odin", "Deathscyther", "ChoZ Spriggan", "Psychic Phantom"];
    ...
    //{
    //client.bossys.get('bosssystem').run(message, prefix, db)
    //};
...
//Client
client.on('ready', () => {
    console.log('Neoblade is online.');
