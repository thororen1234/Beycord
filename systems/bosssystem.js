        ...
        if (boss.hp <= 0 || boss.stamina <= 0) {
            ...
            if (diff < latest.time) {
                ...
                let embed = new Discord.MessageEmbed()
                    ...
                    .setThumbnail(boss.image2)
                    .setTimestamp();
                executeFromURL(client, process.env.WEBHOOKURL, { embeds: [embed], embed: embed });
...
module.exports.run = async (client, message, prefix, player, db, bosss) => {
    ...
    let alert = new Discord.MessageEmbed()
        .setTitle("<:alert:866458990559625247>***ALERT!*** A boss is about to spawn in 10 minutes!")
        .setColor("#eb1465")
        ...
        .setDescription("React to this message with <:notify:844767097664831508> if you wish to get notified 1 minute before the boss battle starts.\n*Get your strongest and most trustworthy Bey ready, bladers!*")
        .setTimestamp();
    let alertembed = await message.channel.createMessage({ embed: alert }).catch(err => {
        return;
    });
    alertembed.addReaction(":notify:844767097664831508")
    let notifs = await ReactionHandler.collectReactions(alertembed, userid => true, { time: 540000 });
    notifs.forEach(async reacted => {
        if (reacted.emoji.id && reacted.emoji.id === "844767097664831508" && !boss.notifs.includes(reacted.userID)) {
