...
async function awaitMoves(message, opponent1, opponent2, player, prefix, client, logger) {
  ...
  function end(winner, loser, finish, player) {
    ...
    let webhookembed = new Discord.MessageEmbed()
      ...
      .setTimestamp()
      .setColor("#ff7600");
    client.executeWebhook("1032068431513657446", "W0YQJac0JVYek7Ci8qEGCyEx8znbcy-WEhupcpTncunxSNg-rYr7iKl4fTl_KmnKRrsX", { embeds: [webhookembed] }).catch(err => { console.error(err) });
    return;
  }
  chooseMove();
}
...
module.exports.run = async (client, message, args, player, prefix, opponent1, opponent2, msg) => {
  ...
  let jimps = [];
  let versus = "./assets/versus.jpg";
  let chance = Math.round(Math.random() * 100);
  if (chance < 11) versus = "/./assets/menacing2.jpg";
  let images = ["./assets/bbackground.jpg", versus, opponent1.bey.image, opponent2.bey.image];
  ...
  Promise.all(jimps).then(data => {
    ...
    data[0].write(`./tempimages/${opponent1.id}-${opponent2.id}.png`, () => {
