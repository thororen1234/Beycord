...
class GiftBox extends Item {
    ...
    async use(client, message, args, prefix, iindex, db){
        ...
        switch(args[1].toLowerCase()){
            case "gift":
                let box = stats.items[parseInt(args[0])-1];
                ...
                if(user.id == message.author.id) return message.reply("please at least tell a friend to pay <:valtz:863052675968925716>10000 to gift you nothingness instead of trying to gift yourself nothing. *Wait what.*");
                ...
                .setTimestamp()
                .setColor("#ff0090");
                client.executeWebhook("890650294470447134", "Cw8NGFdVQPJKdUWB-eCRhpdPlD-g09ftApbsfvzgTAjCs3fv4KvhbbH8MpPKn4TNDgBP", {embeds: [webhookembed]}).catch(err => {console.error(err)});
            break;
            case "view":
                let box2 = stats.items[parseInt(args[0])-1];
                let cost2 = 10;
                for(var i = 0; i < box2.beys.length; i++){
                    ...
                    else if(client.blackbeys.has(bey.name)) rarity = "Shadow";
                    else rarity = "Exclusive";
                    switch(rarity){
                        case "Common":
                            cost2 = cost2 + 10;
            ...
            case "bey":
                ...
                stats.items[parseInt(args[0])-1].beys.push(stats.beys[bindex]);
            ...
            case "part":
                ...
                stats.items[parseInt(args[0])-1].parts.push(stats.beyparts[bindex2]);
            ...
            case "item":
                ...
                if(bindex3 == parseInt(args[0])-1) return message.channel.createMessage(`You flipped the gift box as if you're trying to add the gift box into the gift box, very nice.`);
                ...
                if(stats.items[bindex3].name == "Gift Box"){
                    for(var i = 0; i < stats.items[bindex3].items.length; i++){
                        stats.items[parseInt(args[0])-1].items.push(stats.items[bindex3].items[i]);
                    }
                    for(var i = 0; i < stats.items[bindex3].beys.length; i++){
                        stats.items[parseInt(args[0])-1].beys.push(stats.items[bindex3].beys[i]);
                    }
                    for(var i = 0; i < stats.items[bindex3].parts.length; i++){
                        stats.items[parseInt(args[0])-1].parts.push(stats.items[bindex3].parts[i]);
                    }
                    ...
                    stats.items[parseInt(args[0])-1].items.push(stats.items[bindex3]);
                }
