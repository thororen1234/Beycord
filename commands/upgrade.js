    ...
    switch (gen) {
        case 1:
            ...
            Promise.all(jimps).then(data => {
                ...
                let written = await data[2].writeAsync(`./tempimages/${message.author.id}gen2.png`);
            });
            ...
            let gen2image;
            try { gen2image = fs.readFileSync(`./tempimages/${message.author.id}gen2.png`) } catch (err) { message.channel.createMessage(`*Upgrade failed!* Please try again.`); db.collection("users").updateOne({ _id: message.author.id }, { $set: { stars: stats.stars - 1 } }); gen2image = null; };
            if (gen2image) {
                ...
                fs.unlink(`./tempimages/${message.author.id}gen2.png`, (err) => {
        ...
        case 2:
            ...
            Promise.all(jimps2).then(data => {
                ...
                data[2].composite(data[1], 0, 0);
                let written = await data[2].writeAsync(`./tempimages/${message.author.id}gen3.png`);
            });
            //    let wait2 = await client.delay(5000);
            let gen3image;
            try { gen3image = fs.readFileSync(`./tempimages/${message.author.id}gen3.png`) } catch (err) { message.channel.createMessage(`*Upgrade failed!* Please try again.`); db.collection("users").updateOne({ _id: message.author.id }, { $set: { stars: stats.stars - 1 } }); gen3image = null; };
            if (gen3image) {
                ...
                fs.unlink(`./tempimages/${message.author.id}gen3.png`, (err) => {
                    if (err) console.log(err);
                });
        ...
        case 3:
            ...
            Promise.all(jimps3).then(data => {
                ...
                let written = await data[2].writeAsync(`./tempimages/${message.author.id}gen4.png`);
            });
            //    let wait3 = await client.delay(5000);
            let gen4image;
            try { gen4image = fs.readFileSync(`./tempimages/${message.author.id}gen4.png`) } catch (err) { message.channel.createMessage(`*Upgrade failed!* Please try again.`); db.collection("users").updateOne({ _id: message.author.id }, { $set: { stars: stats.stars - 1 } }); gen4image = null; };
            if (gen4image) {
                ...
                fs.unlink(`./tempimages/${message.author.id}gen4.png`, (err) => {
                    if (err) console.log(err);
                });
        ...
        case 4:
            ...
            Promise.all(jimps4).then(data => {
                ...
                let written = await data[2].writeAsync(`./tempimages/${message.author.id}gen5.png`);
            });

            let gen5image;
            try { gen5image = fs.readFileSync(`./tempimages/${message.author.id}gen5.png`) } catch (err) { message.channel.createMessage(`*Upgrade failed!* Please try again.`); db.collection("users").updateOne({ _id: message.author.id }, { $set: { stars: stats.stars - 1 } }); gen5image = null; };
            if (gen5image) {
                ...
                fs.unlink(`./tempimages/${message.author.id}gen5.gif`, (err) => {
                    if (err) console.log(err);
                });
                fs.unlink(`./tempimages/${message.author.id}gen5.png`, (err) => {
                    if (err) console.log(err);
                });
