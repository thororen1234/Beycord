...
module.exports.run = async (client, message, args, prefix, player, db) => {
  ...
  let cost = 10;
  if (client.commonbeys.name) cost += 10;
  else if (client.rarebeys.name) cost += 25;
  else if (client.legendarybeys.name) cost += 50;
  else if (client.availablebeys.name) cost += 500;
  else if (client.blackbeys.name) cost += 1000;
  else cost += 1000;
  if (client.commonbeys.name) cost += 10;
  else if (client.rarebeys.name) cost += 25;
  else if (client.legendarybeys.name) cost += 50;
  ...
  if (stats.coins < halved || stats2.coins < halved) return message.channel.createMessage(`Insufficient Valtz. Make sure both players have enough Valtz to pay for the trading fees. (<:valtz:844765554237243403>${halved})`)
  let jimps = [];
  let images = ["C:/beycordplus/images/tbackground.jpg", bey1.image, bey2.image];
