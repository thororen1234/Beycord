...
module.exports.run = async (client, message, args, prefix, player, db) => {
  ...
  Promise.all(jimps).then(data => {
    ...
    data[0].write(`/home/container/tempimages/${message.author.id}-${tuser.id}trade.png`, () => {
      console.log("Image written!")
    });
  });
  //let wait = await client.delay(1500);
  let tradeimage = fs.readFileSync(`/home/container/tempimages/${message.author.id}-${tuser.id}trade.png`);
