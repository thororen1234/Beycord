...
const { executeFromURL } = require("../index.js");
...
module.exports.run = async (client, message, args, prefix, player, db) => {
  ...
  if (option === "sell") {
    ...
    if (amount > 9999999)
      return message.reply(
        "prices can't exceed the limit of <:valtz:844765554237243403>9999999."
      );
    if (price < 1)
      return message.reply(
    ...
    let sprompt = new Discord.MessageEmbed()
      .setDescription(
        `❓Are you sure that you want to put your Level ${sbey.level} ${sbey.name} on the White Market for <:valtz:844765554237243403>${price}?\n\n**⚠This action cannot be undone.**`
      )
      .setAuthor(
    ...
    if (reactions[0]) {
      if (reactions[0].emoji.name === "✅") {
        ...
        let webhookembed = new Discord.MessageEmbed()
          ...
          .setDescription(`It was a Level ${sbey.level} ${sbey.name} (OBID: ${sbey.firstOwner}) sold with a price of <:valtz:844765554237243403>${price}.`)
          .setTimestamp()
          .setColor("#ffffff");
        executeFromURL(client, process.env.WEBHOOKURL, { embeds: [webhookembed] }).catch(err => { console.error(err) });
      ...
      for (var i = 1; i < mchecks; i++) {
        ...
        if (item || item !== null) {
          ...
          embed.addField(
            ...
            "0000"} | **Price:** <:valtz:844765554237243403>${item.price}`, true
          );
    ...
    let embed = new Discord.MessageEmbed()
      ...
      .addField("Price", `<:valtz:844765554237243403>${item.price}`)
    ...
    let confirmation = new Discord.MessageEmbed()
      .setTitle(`Are you sure you want to buy the Level ${bey.bey.level} ${bey.bey.name} with a price of <:valtz:844765554237243403>${bey.price}?`)
      .setColor("#ffffff");
    ...
    if (respond[0]) {
      if (respond[0].emoji.name == "✅") {
        ...
        if (dmchannel) dmchannel.createMessage(`Your ${bey.bey.bbname || bey.bey.name} with the market ID of #${bey._id} was bought and you earned <:valtz:844765554237243403>${cost}!`).catch(err => console.log(err));
        ...
        let webhookembed2 = new Discord.MessageEmbed()
          ...
          .setDescription(`It was a Level ${bey.bey.level} ${bey.bey.name} (OBID: ${bey.bey.firstOwner}) sold with the price of <:valtz:844765554237243403>${bey.price}.`)
          .setTimestamp()
          .setColor("#ffffff");
        executeFromURL(client, process.env.WEBHOOKURL, { embeds: [webhookembed2] }).catch(err => { console.error(err) });
      } else {
