...
async function awaitMoves(message, opponent1, opponent2, player, prefix, client, logger) {
  ...
  let battleimage = fs.readFileSync(`/home/container/tempimages/${opponent1.id}-${opponent2.id}.png`);
  ...
  let checkdeath = setInterval(() => {
    if (opponent1.stability <= 0) {
      fs.unlink(`/home/container/tempimages/${opponent1.id}-${opponent2.id}.png`, (err) => {
  ...
  function end(winner, loser, finish, player) {
    ...
    if (finish !== "time-out" && finish !== "cancel") {
      ...
      if (bchance <= 5) {
        if (loser.id !== "501005337373966336") loser.stats.beys[loser.stats.main].broken = true;
...
module.exports.run = async (client, message, args, player, prefix, opponent1, opponent2, msg) => {
  ...
  if (stats1.beys[stats1.main].name === "Guardian Kerbeus Red Ver. <:haoyunshu:844767503813050369>") stats1.beys[stats1.main].name = "Guardian Kerbeus Red Ver. HYS";
  if (stats2.beys[stats2.main].name === "Guardian Kerbeus Red Ver. <:haoyunshu:844767503813050369>") stats2.beys[stats2.main].name = "Guardian Kerbeus Red Ver. HYS";
  ...
  if (opponent2.id === "842641944093392908") {
  ...
  let jimps = [];
  let versus = "/home/container/commands/versus.jpg";
  let chance = Math.round(Math.random() * 100);
  if (chance < 11) versus = "//home/container/commands/menacing2.jpg";
  let images = ["/home/container/commands/bbackground.jpg", versus, opponent1.bey.image, opponent2.bey.image];
  ...
  Promise.all(jimps).then(data => {
    ...
    data[0].write(`/home/container/tempimages/${opponent1.id}-${opponent2.id}.png`, () => {
