...
async function awaitMoves(message, opponent1, opponent2, player, prefix, client, logger) {
  ...
  let battleimage = fs.readFileSync(`C:/beycordplus/tempimages/${opponent1.id}-${opponent2.id}.png`);
  ...
  let checkdeath = setInterval(() => {
    if (opponent1.stability <= 0) {
      fs.unlink(`C:/beycordplus/tempimages/${opponent1.id}-${opponent2.id}.png`, (err) => {
...
module.exports.run = async (client, message, args, player, prefix, opponent1, opponent2, msg) => {
  ...
  let jimps = [];
  let versus = "C:/beycordplus/images/versus.png";
  let chance = Math.round(Math.random() * 100);
  if (chance < 11) versus = "C:/beycordplus/images/menacing2.png";
  let images = ["C:/beycordplus/images/bbackground.jpg", versus, opponent1.bey.image, opponent2.bey.image];
  ...
  Promise.all(jimps).then(data => {
    ...
    data[0].write(`C:/beycordplus/tempimages/${opponent1.id}-${opponent2.id}.png`, () => {
