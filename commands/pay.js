...
module.exports.run = async (client, message, args, prefix, player, db) => {
  ...
  message.channel.createMessage(`✅ <:valtz:844765554237243403>${amount} successfully paid to ${user.effectiveName}.`);
  let webhookembed2 = new Discord.MessageEmbed()
    .setTitle(`${message.author.username}#${message.author.discriminator} (${message.author.id}) paid <:valtz:844765554237243403>${amount} to ${user.username}#${user.discriminator} (${user.id})!`)
    .setTimestamp()
    .setColor("#7f7fff");
