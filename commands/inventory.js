...
module.exports.run = async (client, message, args, prefix, ok, db) => {
  ...
  .setTimestamp();
    for(var i in inv){
      ...
      if(inv[i].level === 0) iinfo += "<:level0:999800926850261043> | ";
      if(client.blackbeys.has(inv[i].name) || inv[i].name === "Black Perfect Phoenix" || inv[i].name === "Dark Perfect Phoenix") iinfo += "<:black:721678218859511829> | ";
      let type = "<:balance:999836011863478335>";
      switch(inv[i].type){
        case "Attack":
          type = "<:attack:999835677329985566>";
        break;
        case "Defense":
          type = "<:defense:999835957018767390>";
        break;
        case "Stamina":
          type = "<:stamina:999835842270986282>";
        break;
        default:
          type = "<:balance:999836011863478335>";
      }
