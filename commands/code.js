...
module.exports.run = async (client, message, args, prefix, player, db) => {
    ...
    let doc2 = await db.collection("users").findOne({ _id: message.author.id }, {redeemed:codec});
    let validate = check(doc2, codec);
    console.log(doc.redeemed, codec, validate);
    if (doc2 && validate) return message.reply("You already redeemed the code.");
